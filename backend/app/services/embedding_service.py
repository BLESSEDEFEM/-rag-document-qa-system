...
class EmbeddingService:
    ...
    def generate_embedding(self, text):
        ...
        Returns dummy embedding - actual embedding happens in Pinecone.
        """
        if not text:
            return [0.0] * 768  # Return dummy 768-dim vector
        
        logger.info("Query embedding will be generated by Pinecone during search")
        return [0.0] * 768  # Dummy vector - Pinecone embeds during query
    
    def generate_embeddings(self, texts):
        """
        Generate embeddings for multiple texts.
        Returns dummy embeddings - actual embedding happens in Pinecone.
        """
        if not texts:
            return None
        
        logger.info(f"Embeddings will be generated by Pinecone for {len(texts)} texts")
        return [[0.0] * 768 for _ in texts]  # Dummy vectors
